...
- **PO calls — the 4 build blockers RESOLVED 2026-06-25** (detail + consequences in `V3_BUILD_PLAN.md` §0): **window → build the whole lane now** (V3.3+ still waits on Lane C as a real dependency, not a window choice); **col-D format → ISO `YYYY-MM-DD`** everywhere (`parseDate` stays as-is; both write surfaces stay ISO via Lane C's helper); **days 3–7 calendar gap → the coming-up strip carries calendar events** (3–7d, 📆-tagged); **love-note exposure → Cloudflare Tunnel** (+ a 4th `pages.yml` sed / `DASHBOARD_LOVENOTE_URL` secret / 4th config-smoke anchor). **Still open (per-slice):** Timeline milestone-inclusion rule + Domain→category map · Education's surface home · `--blue` token fate · JS-test-harness now vs deferred · love-note voice phase (text first) · love-note on-box storage shape · does account-switch need UI beyond reusing the sign-in flow.
