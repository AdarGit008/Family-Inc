...
| 11 | **M6.3/M6.4 acceptance** — ✅ accepted & closed **2026-06-28** (88% coverage report-first; summarizer 0 ALERT-FP/7d) | high | M | low | ✅ done |
| 12 | **classifier-fp-metric** (human-mark channel) | med | M | low | after 06-26 |
| 12.1 | **finance-local-rules-overlay** — box-local rules file for household-specific merchants (a local-grocery-class payee can't enter the public seed; surfaced at the M6 close) — §3.7 | med | S | low | **revisit 2026-07-12** |
...
**Sibling lane — finance-local-rules-overlay (new, after 06-26; rank 12.1).** Surfaced at the M6 close (2026-06-28): a **household-specific local merchant** (e.g. a local grocery that recurs in the blanks) categorizes nowhere — `load_rules` reads only the committed `seeds/14_…csv`, which is **public-portfolio-safe by construction** (national brands + generic labels only, *never* a household-specific payee — committing one both breaks the file's invariant and leaks a location signal). **Contract.** Extend `load_rules` to read the committed seed **plus** an optional **gitignored on-box** file (e.g. `/etc/family-inc/finance_rules.local.csv` or a `seeds/*.local.csv` the `.gitignore` already excludes), local taking precedence, same `pattern,category` format; the local file holds the household-specific merchants and never enters git. Hermetic + a test that the overlay merges and local wins; the public seed stays the sole source of the **LLM vocab** (the local file maps only, adds no new category). Re-run the backfill after it lands to lift coverage past the structural floor. **Check-in (milestone-review-set 2026-06-28): revisit 2026-07-12** — after ~2 weeks of live data, read whether the recurring local-grocery blanks *materially* undercount `Finance-Budget` Groceries actuals; if yes, pull the overlay forward, else confirm the deferral. (A `bank_creds.json` string-prefix override was considered as a temporary bridge and **rejected** — it's a degenerate, untested version of this same overlay; build the real thing if the check-in says it matters.) *Open: file location (`/etc/family-inc` vs a gitignored seed) · whether the vocab-guard test binds only the committed seed (recommend: yes).*
