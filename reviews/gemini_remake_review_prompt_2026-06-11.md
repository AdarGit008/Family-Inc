INFO review: assembled prompt: 6 files, 56,958 chars (~14,239 tokens)
INFO review:   attached: SPEC.md (18,825 chars)
INFO review:   attached: ENGINEERING.md (10,990 chars)
INFO review:   attached: DESIGN.md (9,650 chars)
INFO review:   attached: DECISIONS.md (4,352 chars)
INFO review:   attached: BACKLOG.md (3,974 chars)
INFO review:   attached: CLAUDE.md (4,158 chars)
...
read Reminders where Status ∈ {Pending, Snoozed, Overdue}
  skip if WriteQueue_Tombstone within 6h        → log skipped_due_to_tombstone
...
Dashboard stamps `WriteQueue_Tombstone` on every write; engine skips rows tombstoned <6h. Residual accepted race: phone offline >6h with a queued tap (tombstone is written at flush) → at most one duplicate alert; the flush itself is idempotent. Window tunable; widen to 12h if observed.
...
2. **Email digest** to both adults — automatic when bridge is silent >24h; same renderer, no budget change.
...
| Secrets | `/etc/family-inc/` mode 600 | `service-account.json`, `env` (ANTHROPIC_API_KEY), `recipients.json` |
...
5. Pair Baileys: `systemctl stop family-bridge && node bridge/baileys_listener.js` interactively once, scan QR, restart service.
...
- Self-reporting: the weekly briefing includes one system line — "7/7 runs green · 41 messages classified · ₪6.10 LLM spend"; any fail-flag or stale heartbeat replaces it with a warning. The humans never check logs unless the briefing tells them to.
