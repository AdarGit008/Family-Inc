...
- **When:** 2026-06-19T10:33:10
- **Provider:** DeepSeek (`deepseek-chat`)
- **Elapsed:** 34.7s
- **Attached files (8):**
  - `CLAUDE.md` (5,311 chars)
  - `SPEC.md` (37,779 chars)
  - `BACKLOG.md` (13,777 chars)
  - `ENGINEERING.md` (13,878 chars)
  - `automation/daily_digest.py` (29,490 chars)
  - `automation/lib/outbox.py` (15,696 chars)
...
1. **[HIGH] `_reconcile_locked` reads `current = {r.row: r for r in sheet.read_reminders(sheet_path)}` once at the top, then iterates `confirmed` entries. A confirmed entry may span many `rows`; if the Sheet changes between processing the first and last row of the same digest (e.g., a dashboard write sets a row to `Done`), the later row uses a stale `current` snapshot.**
   *File:* `automation/daily_digest.py`, lines `_reconcile_locked`, the `for row in e.get("rows", [])` loop.
   *Fix:* either re-read `current[row["row"]]` per row inside the loop, or (less expensive) move the `curr = current.get(row["row"])` call inside the loop so every row re-checks the live dict. The problem is not theoretical: a dashboard write that lands between the `read_reminders` call and the stamp is masked if the snapshot is stale for just that row.

2. **[MEDIUM] `_record_pending` persists `fail_lines` per recipient entry, but `_clear_fail_flag` in `_reconcile_locked` processes the entire entry's `reported_fail_lines` list as one bulk remove from the flag file.** A confirmed digest will clear ALL its recorded fail-lines, even lines that only belonged to *one* of the two recipients. The flag file is global, not per-recipient, so clearing per-recipient lines independently across two entries for the same digest can remove a line that should survive because the *other* recipient's entry was not confirmed.
   *File:* `automation/daily_digest.py`, `_reconcile_locked` → `_clear_fail_flag(e.get("reported_fail_lines", []))` calling the same `_clear_fail_flag` function used elsewhere.
   *Suggestion:* deduplicate the `reported_fail_lines` across both recipients of the same digest before a single clear call, or track which lines belong to which recipient.

3. **[MEDIUM] The 48h stale horizon (`DIGEST_PENDING_STALE_HOURS`) resets if the entry's `queued_at` is touched by an intervening delivery attempt (e.g., an hourly outbox poll updates the file mtime, but the pending JSONL content remains unchanged).** The `_pending_age_hours` function computes age from `queued_at` (the `jsonl` value), not from `mtime` — so a simple `touch` does not reset it. However, if the operator ever manually re-runs the digest and a new pending entry is appended with the same `msg_id`, the age resets. The spec says "stale" counts from queue time. That's correct behavior for a re-queued digest, but a stale re-run of the same digest repopulates the pending file with a fresh timestamp, hiding the original failure.
   *File:* `automation/daily_digest.py` `_pending_age_hours` function, and `run()` `_record_pending` call.
   *Risk:* masked silent-loss. Not a blocker, but the reconciler should detect that the *same* `msg_id` is already pending and not reset the age.
   *Fix:* check if a pending entry with the same `msg_id`+`recipient` exists before appending; if so, preserve the original `queued_at` (or skip recording).

4. **[MEDIUM] `_reconcile_locked`'s tombstone deference on `confirmed` entries: "if *any* confirmed row has a fresh tombstone, defer the *whole* confirmed set."** This is overcautious — it holds back perfectly confirmable rows for a different row's in-flight write. A dashboard write to row 10 doesn't mean rows 3 and 7 of the same digest are unsafe to stamp.
   *File:* `automation/daily_digest.py`, `_reconcile_locked`, the `if confirmed and any(…)` block.
   *Fix:* only defer the specific rows whose tombstone is fresh and their corresponding stamp writes; stamp the rest.

5. **[LOW] `stamp_cell_writes` in `reminders_engine.py` returns two `CellWrite` objects (Last Sent and Status). The `_apply_reminder_writes` function counts *distinct rows* via `len({w.row for w in writes})`. If one row's write succeeds and the other fails (partial batch), the `return len({w.row for w in writes})` still counts the row as stamped — because the row *appeared* in the write set.**
   *File:* `automation/daily_digest.py` `_apply_reminder_writes` and `stamp_cell_writes` in `automation/reminders_engine.py`.
   *Risk:* the reconciler thinks a row was stamped, but only half the column update landed. The deferred-alert consumption and fail-flag clear still proceed. Partial-stamped rows are under-stamped, not over-stamped (the pre-existing sheet state is overwritten by the one cell that went through). This may leave Status = Sent but Last Sent empty, or vice versa.
   *Fix:* have `_apply_reminder_writes` return the set of rows for which *both* writes succeeded (gspread batch update is atomic per call, so this is only an issue if there are two `batchUpdate` calls — the current code makes one call, so the risk is low enough to be a **defense** rather than a fix).

### Missed alternatives

1. **Pending-file lock per digest not per run** — a single fcntl lock across all pending operations is coarse; a per-digest lock would keep the reconciler from blocking a concurrent `_record_pending` for a different digest.
2. **Replace the `reported_fail_lines` track with an absolute "clear after N successful digests" rule** — bypasses the per-recipient association problem entirely by clearing the flag file only when ALL recipients of a given digest confirm.
3. **A "soft fail" column on the pending entry itself** — if the bridge never confirms but the SMTP fallback later succeeds for the same recipient (e.g., the next day's digest delivers while the pending entry from yesterday is still unconfirmed), the reconciler could look at the delivery log instead of only `whatsapp_sent.jsonl`.
4. **Move the `Sheet.read_reminders` call inside the per-entry loop** — avoids the stale snapshot problem described in Concern #1 at the cost of N + 1 reads (but the read is already a cached dict after the first call, so the cost is zero).
5. **Kick a pending entry back to "unattempted" after the stale-dropped threshold** — the entry is dropped now (safe), but it's not surfaced anywhere visible; a follow-on re-fire of reminders is good, but a weekly briefing line would close the loop (already noted as an open follow-on, so this is a *reminder*, not a new path).

### Affirmations

1. **Rejecting the bounded in-run wait** is the correct call. The cross-run reconcile design is the right tradeoff: it decouples the digest's schedule from the bridge's asynchronous timing, avoids double-delivery risk, and the re-fire heuristic (reminders stay unstamped → re-fire) is the correct "fail loud" behavior.
2. **The `done/reschedule/recurrence-bump/tombstone` re-read guards** in `_reconcile_locked` are thorough and handle a real data-race pattern. The decision to check `cur.due.isoformat() != stored_due` against the snapshot stored at queue time correctly prevents stale-stamp resurrection.
3. **Dating Last Sent to the digest's `queued_at`** (not the reconcile day) is the right call for traceability and to prevent a +1-day skew that would confuse the Last-Sent guard.
4. **The `stamp_cell_writes` factoring** so that both the inline SMTP path and the cross-run reconcile path call the same function is a solid single-source-of-truth pattern.

### Concrete suggestions

1. **In `_reconcile_locked`, replace the single `current = {r.row: r for r in sheet.read_reminders(sheet_path)}` with a per-row re-read inside the confirmed loop.**  
   Change the `for e in confirmed:` inner loop from:  
   `cur = current.get(row["row"])`  
   to:  
   `cur = sheet.read_reminders(sheet_path).get(row["row"])`  
   (or an optimizing re-read if the function is expensive). This eliminates the stale snapshot risk (Concern #1) and costs two Sheet reads per digest at most.

2. **In `_reconcile_locked`, deduplicate `reported_fail_lines` across both recipients before calling `_clear_fail_flag`.**  
   Before the `for e in confirmed:` loop, collect all `reported_fail_lines` from all confirmed entries of the same `msg_id` into a single set, then call `_clear_fail_flag(list(set_of_lines))` once. This resolves the per-recipient clearance problem (Concern #2).

3. **In `_record_pending` (or in `run`), check whether a pending entry with the same `msg_id`+`recipient` already exists before appending.**  
   If one exists, skip the append (the original `queued_at` is already recorded, so the age computation stays truthful). This prevents the stale-horizon reset (Concern #3).

4. **In `_reconcile_locked`, split the "tombstone defers all confirmed rows" logic into per-row deferral.**  
   Change the block from:  
   `if confirmed and any(…)` → defer entire `confirmed`  
   to:  
   ```
   still_ok = [e for e in confirmed if not any(
       row["row"] in current and engine.is_tombstoned(current[row["row"]], now)
       for row in e.get("rows", []))]
   deferred += [e for e in confirmed if e not in still_ok]
   confirmed = still_ok
   ```
   This lets confirmed, non-tombstoned rows proceed (Concern #4).

### One question for the team

**Adar + Shanee: If the bridge confirms a digest for Adar but not for Shanee (partial delivery), do you want the reconcile to stamp the rows visible to both (which clobbers the un-stamped entries for Shanee's pending record), or defer the full batch until both recipients confirm?**  
(The current design stamps per-recipient independently inside the same pending entry—the code splits by recipient into separate entries earlier so it's safe—but the fail-flag clear and deferred-consumption happen per entry, so a partial confirmation will clear lines that belong to the still-unconfirmed other half.)
