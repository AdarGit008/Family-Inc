...
1. **BROKEN OBSERVABILITY CONTINUITY — fail flag cleared by “next *delivered* digest,” but if the digest itself is the unit that failed, the flag clears without being reported.**  
   File: `SPEC.md` §9 (failure modes table, “Bridge logged out” row / “Email fallback” row) + `ENGINEERING.md` §5 (fail-flag unit description).  
   The current semantics: any unit’s `OnFailure=` appends to `logs/fail.flag`; the *next delivered digest* reports and clears it. If the digest timer itself fails (e.g., `family-digest` service crashes after `OnFailure=` appends `family-digest@.service`), the next *successful* digest — which is the unit that just failed — will clear the flag without having gotten a chance to report it, because the flag was appended *by* the failure. This is a self-healing loop that eats its own failure log.  
   **Severity: HIGH** — this is a concrete failure of “fail loud” (SPEC §3.6) for the system’s most critical message. The `family-digest` failure disappears without human notice.

2. **EMAIL FALLBACK HAS NO MONITORING — no record exists that the fallback fired.**  
   File: `lib/mailer.py` (described in the session changes) + `SPEC.md` §10.2.  
   When the digest degrades to SMTP, it stamps normally and notes “delivered by email” in the body, but no flag, no `logs/` line, and no independent record surfaces. The *only* detection path is if a human reads the email subject or sees the message. If the bridge recovers in time for tomorrow’s digest, the one-time fallback is invisible in the system layer. The weekly briefing’s “7/7 runs green” line would report a green digest even though it went via email. This is a monitoring blind spot for exactly the failure mode the fallback exists for.

3. **`TODO` — `backup.sh` PRUNE POLICY REFERENCED IN `ENGINEERING.md` §5 BUT VALUE IS ZERO — the 90-day prune is not a constant, it’s a shell argument that could drift.**  
   File: `ENGINEERING.md` §5 (“Sunday 03:00” row mentions 90-day prune) + `deploy/backup.sh` (session-change description says “90-day prune”).  
   The prune window exists only as a literal in the shell script. If the team ever changes the retention policy (e.g., to 30 days when storage runs low, or to 180 after a scare), there is no single source of truth to change — you must edit the shell script itself. All other policy constants live in `config.toml` or `/etc/family-inc/env`; the backup prune window should too.

4. **`WHAT?` — `recipients.json` FALLBACK IS BROKEN FOR THE BRIDGE — the bridge is Node, and Node code was not mentioned in the session as updated.**  
   File: `automation/bridge/baileys_listener.js` (reference from `ENGINEERING.md` §1 layout) + session-change line “`recipients.json` read from `/etc/family-inc/` first, bridge-dir fallback for dev”.  
   The session change only updated the *Python* side to check `/etc/family-inc/` first. The bridge (Node) reads `recipients.json` from its own `bridge/state/` directory. Unless the bridge was also updated in this session (not listed), the bridge will never check `/etc/family-inc/recipients.json` — it continues reading from the non-fallback directory, which is either missing in production (if no dev copy was placed) or stale. The production VPS will have a non-functional bridge unless a manual symlink or separate Node change was made.

5. **`BREAKING` — `deploy.sh` ABORTS ON RED TESTS, BUT TESTS INCLUDE THE SEED-CSV IMPORT PATH — those CSVs are gitignored and absent from the deployed checkout.**  
   File: `ENGINEERING.md` §6 (deployment: `uv run pytest -q`) + `seeds/Reminders_Import_M3.csv` (gitignored, per session change).  
   The test suite (191 tests) includes tests that exercise the engine against the seed xlsx *when no live Sheet ID is set*. Those tests pass in CI because the seed CSV exists there. On first deployment to a fresh VPS before the seed is imported, there may be no seed CSV, or the seed CSV may be a different file than the test expects. The test suite could fail on the VPS for reasons unrelated to code health, blocking the deploy script entirely. The team must confirm the seed CSV is present or the tests are skipped in the production deploy path.

### Missed alternatives

- Use a dedicated `healthchecks.io` ping for the digest that fires *after* the SMTP fallback, so email-degraded delivery is still a green check at a heartbeat URL, rather than invisible.
- Read `recipients.json` from a single path with a symlink during provisioning, eliminating the fallback entirely and forcing one source of truth.
- Store the backup prune window in `/etc/family-inc/env` (e.g., `BACKUP_RETENTION_DAYS=90`) and read it in `backup.sh`.
- Use a second flag file (`logs/fallback_fired.flag`) that the digest appends when it sends via email, and the weekly briefing clears + reports.
- Add a health check inside `deploy.sh` that validates the seed CSV exists before running tests, or use `--deselect` to skip integration tests that require it.

### Affirmations

- **Fail-flag loop closing via the *delivered* digest.** Despite concern #1 above, the *intent* is correct: the flag clears only on a real delivery attempt, not on a failed one. The model of “the thing that delivers gets to clear the flag” is sound for all units except the digest itself.
- **Email fallback framed as transport substitution, not outbox bypass.** This is a clean architectural boundary: the content has already passed the budget and policy checks in the outbox; the email path is a drop-in replacement for the Baileys send, not an alternative queue. This was the right call.
- **Pages-via-Actions with `config.js` generated from secrets.** This avoids checking OAuth IDs into the repo while still enabling static-site deployment from a public repo. The flow is auditable in the workflow YAML.
- **One-sudoers-line for bridge restart.** A proven pattern; limits blast radius from a compromised `familyinc` user. Good engineering hygiene for a single-VPS appliance.
- **Tests 172→191 with dedicated `test_mailer.py`.** Including the SMTP fake, degrade paths, and fail-flag semantics. This is the correct investment for a critical fallback path.

### Concrete suggestions

1. **Replace** “the next *delivered* digest reports and clears `logs/fail.flag`” **with** “the next *delivered* digest ignores `fail.flag` lines that match its own unit name, reports all others, and clears the file on completion.”  
   In `deploy/systemd/family-fail-flag@.service`, pass the failing unit name as `%n` to the template script. The digest script reads the flag, filters out lines matching itself, prepends the remaining failures to the digest body, then clears the file.  
   **Because:** this prevents the digest from self-reporting silence after a crash.

2. **Append** a line to `logs/digest_log.csv` on every digest run that records the transport used (`baileys`, `smtp`, `queued`).  
   In `lib/mailer.py` (or `automation/daily_digest.py`), log the transport outcome at the same level as the fail-flag.  
   **Because:** the transport choice is the single most important observability signal for the delivery pipeline, and it is currently invisible to the system’s self-reporting.

3. **Move** the backup prune window from a literal in `deploy/backup.sh` to a variable read from `/etc/family-inc/env` (e.g., `BACKUP_RETENTION_DAYS=90`).  
   **Because:** all other policy constants live in config; this one doesn’t, and it will drift.

4. **Update** `deploy/systemd/family-bridge.service` to set an `ExecStartPre` script that copies `/etc/family-inc/recipients.json` → `bridge/state/recipients.json` before starting the Node process.  
   **Because:** the bridge reads from a fixed path that is not `/etc/family-inc/`; the Python-side fix in D-027c did nothing for the bridge itself.

5. **Add** a flag to `deploy.sh` or the test invocation to skip tests that require seed CSV files when deploying to a fresh box: `uv run pytest -q --ignore=tests/test_seeds.py` if no seed CSV exists.  
   **Because:** the current test suite will fail on first deploy, blocking the go-live runbook.

### One question for the team

**Adar + Shanee: is an email-delivered digest a “green run” for the purpose of the weekly briefing’s “N/N runs green” line, or does the system need to surface “1/7 runs were email- fallback” so you know the bridge degraded? The current design treats it as green (the digest fired), but the concern is that a degrading bridge becomes invisible until it’s dead for 24h — the fallback may be hiding the ramp. What is the correct reporting posture?**
