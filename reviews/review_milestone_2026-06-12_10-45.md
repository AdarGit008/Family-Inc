...
- **When:** 2026-06-12T10:45:00
- **Provider:** DeepSeek (`deepseek-chat`)
- **Elapsed:** 30.4s
- **Attached files (6):**
  - `CLAUDE.md` (4,860 chars)
  - `SPEC.md` (20,381 chars)
  - `BACKLOG.md` (8,477 chars)
  - `ENGINEERING.md` (13,678 chars)
  - `DESIGN.md` (9,650 chars)
  - `DECISIONS.md` (8,189 chars)
...
1. **CRITICAL: Tombstone re-stamping at flush time introduces a race with the engine's §8.3 skip window.** `SPEC.md §8.3` states the engine skips rows tombstoned <6h. If the dashboard flushes after being offline for 5h, the tombstone is 5h old; the engine might fire at the 5.5h mark, see the 5h-old tombstone, and skip. But the dashboard's UI already shows the row as done. The user sees a stale row. The re-stamp doesn't reset the clock for the engine's guard — it only prevents duplicate flushes from the dashboard itself. The fix is to ensure the engine's skip window uses the *tombstone write time*, not the *flush time*. `lib/sheet.py` must compare against `WriteQueue_Tombstone` cell value, not a local timestamp. Current spec language ("engine skips rows tombstoned <6h") is ambiguous about whose clock. Fix in `SPEC.md §8.3` and `lib/sheet.py` (M2 changes description).

2. **HIGH: Dual schema-drift guard on Reminders writes is asymmetric.** `SPEC.md §7.1` says the guard aborts on *reads AND writes*. If a write hits a drift mid-batch (e.g., column added between rows), aborting the write leaves partial state in the Sheet — some rows stamped, others not. The weekly briefing reports the drift, but the partial write is unreported and may cause double-sends on retry. Either make writes atomic (batchUpdate with precondition that fails fast for the whole batch, then no partial state) or log the partial state for the briefing. Suggest clarifying in `SPEC.md §7.1` that drifts on write abort the batch atomically (or document the accepted partial-state risk as "degrade quiet").

3. **MEDIUM: Settings tab layout conflates user-map entries with chrome config keys.** `SPEC.md §6.4` says `Settings` is Key|Value rows, keys containing `@` are UserMap (email→name), and key `lang` is the chrome default. This means the tab mixes human identities with system configuration. The dashboard reads both via `batchGet`; a corrupted UserMap entry (e.g., key without `@` but email-like) would silently fall through to a config-key lookup. Add a prefix convention (e.g., `user.`) to distinguish identity from config, or use a separate `UserMap` tab. Suggested for `SPEC.md §6.4`.

4. **MEDIUM: Outbox over-budget deferral into tomorrow's digest can cause infinite deferral loop for persistent alerts.** `SPEC.md §7.5` says over-budget alerts are downgraded: appended to tomorrow's digest. If the same alert fires daily (e.g., a weekly lead-time on a high-priority reminder), it could bump against the budget every day, get deferred to the next digest, and never get sent. The weekly "budget is biting" line (SPEC §8.1) catches this after 14 days, but the alert itself remains unsent for that period. Document this edge case in `SPEC.md §7.5` or add a priority override: overdue alerts that have been deferred >3 days bypass budget once (logged as `alert_suppressed_by_budget_released`).

5. **LOW: Seed xlsx headers aligned to §6.1, but `lib/sheet.py` validation on the seed is not documented.** `lib/sheet.py` has a schema-drift guard that aborts on header mismatch. When running against the seed (no live Sheet ID), does it still validate? The M2 changes say "creds-less runs never write the seed," but a header mismatch on the seed would abort the run entirely, preventing local testing. Clarify in `ENGINEERING.md §9` or `lib/sheet.py` header validation that the guard is skipped or soft-warned when `FAMILY_INC_SHEET_ID` is unset.

### Missed alternatives

- Use a single `last_modified_at` cell per row instead of the tombstone column, relying on Sheet's built-in `updated` timestamp for the 6h skip window.
- Outbox budget enforcement via a deduping accumulator (set of `(kind, date, recipient)` tuples) rather than a decrementing counter.
- Reminders engine reading Status = `{Pending, Snoozed, Overdue, Sent}` instead of `∉ {Done, Skipped}`, with Last-Sent guard on Sent rows.
- Replace dual `DoneAt`/`LastDoneBy` columns with a single history row (timestamp + actor + action) in a `Reminders-Log` tab, keeping Reminders clean.
- Implement the reply-ack kind as a no-op stub now (kind=ack, silently dropped) so the bridge can begin stamping ids without sending, easing the v1.1 transition.

### Affirmations

- Tombstone re-stamping at flush time is the correct fix for the race where an offline phone queues a write and the engine fires before the flush completes.
- Outbox chokepoint consolidation (all senders share one budget ledger) closes the double-spend bug without changing the spec — D-015 applied correctly.
- Settings tab as read-only on the dashboard (via `batchGet`, not local config) ensures user-identity resolution survives a cache clear.
- Hebrew templates with dual verb forms matching the dashboard (DESIGN §6) is the right i18n choice for a household where both adults speak Hebrew but may use different register levels.
- Deferring weekly briefing LLM narrative to an unscheduled lane is pragmatic — the deterministic template delivers value now.

### Concrete suggestions

1. Replace "engine skips rows tombstoned <6h" in `SPEC.md §8.3` with "engine compares `WriteQueue_Tombstone` cell value against `now()`; if `cell_value + 6h > now()`, skip." This removes the ambiguity about re-stamping resetting the clock.

2. In `SPEC.md §7.5`, add: "Any single alert that has been deferred by budget >3 consecutive days is released on day 4 as a critical bypass, logged as `alert_released_after_deferral`." This prevents indefinite deferral.

3. In `SPEC.md §7.1` header-validation section, add: "When `FAMILY_INC_SHEET_ID` is not set (seed mode), the guard logs a warning but does not abort; seed runs tolerate header drift to support local testing."

4. In `SPEC.md §6.4`, add a note: "Settings keys containing `@` are UserMap entries. To prevent collisions, all non-UserMap keys (e.g., `lang`) must not contain `@`. Consider adding a `user.` prefix in v1.1 for explicit distinction."

5. Replace "tombstones re-stamped at flush time" in the M2 changes description with "tombstones re-stamped at flush time to the Sheet cell (not to a local variable); the engine always reads the Sheet cell value for the skip-window check." This ensures the implementation matches the intent.

### One question for the team

Given that the dashboard's re-stamped tombstone does not reset the engine's 6h clock (because the engine reads the Sheet cell, not a local timestamp), is the 6h window measured from the original write or from the flush time, and how does the engine distinguish between a tombstone that was just re-stamped (dashboard flush) vs an old one that should still be respected?
...
## Resolution (in-session, 2026-06-12 — Lead Architect; logged as D-026)

| # | Finding | Resolution |
|---|---|---|
| 1 | Tombstone re-stamp "race" (CRITICAL) | **Defend + wording Apply.** Mechanics inverted: the flush writes `now()` INTO the cell; the engine compares the cell to its own clock each run — the 6h window starts at flush by construction. The reviewer's 5h-stale-tombstone scenario cannot occur. Applied their §8.3 wording (one-clock semantics) since the ambiguity invited exactly this misreading. |
| 2 | Partial batch state on write-drift (HIGH) | **Defend + clause Apply.** The guard validates the header BEFORE the single batched write — drift means nothing is written. Mid-batch API failure (rare 5xx) is bounded by idempotent stamps + the same-day Last-Sent guard. Added the validate-before-write clause to §7.1. |
| 3 | Settings tab `user.` prefix / separate tab | **Defend.** Unrecognized keys fall through to nothing (then cfg fallback) — there is no config-key collision path. A prefix convention on a four-row human-edited tab isn't boring tech. Revisit if UserMap grows past the household. |
| 4 | Infinite deferral loop + aging-into-critical | **Defend.** Deferral is consumption: `pop_deferred` folds the body into tomorrow's digest, which goes out daily — delivery is guaranteed on day 2, there is no re-queue loop. Aging deferred alerts into a critical bypass would erode the 2/day principle (criticals are safety-only, D-008); the §8.1 "budget is biting" line is the intended escalation surface. |
| 5 | Soft-warn guard in seed mode | **Defend.** The uniform guard is how this session caught the seed's own header drift. Softening it in dev would stop local testing from enforcing the one contract the guard exists for. The seed ships conformant; an abort on a locally-edited header is the feature. |

Missed alternatives: Sheets has no per-cell modified timestamp reachable from the values API (alt 1 rejected as not-boring); `∉ {Done, Skipped}` chosen over enumerating eligible statuses so future additive statuses stay eligible (alt 3); history-log tab instead of M/N/O is a v2-scale remodel (alt 4); a stub `ack` kind is exactly the SPEC §7.5 change reserved for the v1.1 PO call (alt 5, already in BACKLOG).

Reviewer's question answered by the applied §8.3 wording: the window is measured from the flush (the cell value IS the flush time); the engine never needs to distinguish re-stamped from original — the cell always carries the latest landing.

**Outcome: 2 wording applies, 5 defends, 0 open. No direction changes. M2 close stands.**
