...
- ⬜ **M6.4 — analysis layer.** The on-box rules engine (`seeds/14_Finance_Category_Rules.csv`) populates `Category`/`Cat-Source` at ingest; DeepSeek gap-fills the rules-miss remainder (description + amount only). Briefing + drawer gain per-category spend, month-over-month trends, and actuals-vs-budget. **Build note:** finance writes `Date` as ISO text, but the budget actuals are date-windowed `SUMIFS` against serials — live actuals read ₪0 until reconciled (pick `USER_ENTERED` append *or* a text-compare `SUMIFS`, and verify actuals go non-zero on a real month). Silent delivery; no anomaly detection.
