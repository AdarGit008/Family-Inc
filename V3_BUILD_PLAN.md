...
- **Open calls:** **network exposure** (tunnel/proxy/tailnet + TLS) — **blocker** · auth scheme (access_token→userinfo vs id_token→tokeninfo) · storage shape (flat JSON vs sqlite; where voice bytes live) · sweep cadence · `LOVENOTE_URL` sed-substituted vs plain template (drives the smoke anchor).
