...
# A rules-miss batch sent to the LLM is capped so a pathological import can't
# balloon one prompt; the overflow stays blank this run and gets a fresh try
# next run (the dedup means only genuinely-new txns are ever re-presented).
...
def _gapfill(misses: list[dict], vocab: list[str]) -> None:
    """DeepSeek (or the configured fallback) over the rules-miss remainder.
    Sends ONLY description + amount per txn (§8.6). No vocab / no key / error /
    off-vocab → the txns stay blank. lib/llm is imported lazily so a keyless
    box pays nothing and the import never hinges on the LLM module."""
    if not vocab:
        return
    from automation.lib import llm
    if not llm.available():
        return
    batch = misses[:GAPFILL_MAX_BATCH]
    ...
    raw = llm.complete(prompt, task="categorize", system=system, max_tokens=600,
