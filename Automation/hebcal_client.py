...
We use geonameid 294801 (Haifa) as a proxy for ⟨town⟩ — Hebcal
does not maintain a separate location for ⟨town⟩, but Haifa
shares the same coastal zman to within ~1 minute and is the closest
metro. If you ever move further inland (e.g. Kiryat Tivon proper) swap
to geonameid 293322 (Tivon).

Endpoints used:
  - Shabbat:  https://www.hebcal.com/shabbat/?cfg=json&geonameid=294801&M=on
  - Calendar: https://www.hebcal.com/hebcal?v=1&cfg=json&maj=on&min=on&mod=on
              &nx=on&year=now&month=x&ss=on&mf=on&c=on&geo=geoname
              &geonameid=294801
...
Run modes:
  python hebcal_client.py            # smoke test: this Shabbat + 30d chagim
"""
...
GEONAME_ID = 294801  # Haifa — proxy for ⟨town⟩
...
if __name__ == "__main__":
    ...
    today = date.today()
    print(f"Shabbat times around {today.isoformat()} (Haifa proxy for ⟨town⟩):")
