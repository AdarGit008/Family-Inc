...
- WhatsApp plaintext exists in places we don't fully control — Meta's servers (inherent) and the configured LLM provider — plus the VPS we do. The single permitted external message processor is **DeepSeek**; no others. LLM classification sends one message + up to 3 context messages, never whole threads or cross-group context. *(This is a deliberate privacy-vs-jurisdiction call by the POs: it routes group plaintext through PRC-jurisdiction infra, accepted because the cost is negligible at household volume and every path has a keyless fallback.)*
- **Finance categorization:** DeepSeek may assign a category to the **rules-miss remainder only** — a transaction's **description + amount**, never account numbers, balances, credentials, identifiers, or the whole ledger. The on-box rules engine tags first, so most transactions never leave the box.
...
One wrapper (`lib/llm.py`); model ids in config, not at call sites; per-call cost logged to `logs/llm_costs.csv`. The active provider is chosen by **key presence**: `FAMILY_INC_DEEPSEEK_API_KEY` → DeepSeek (`deepseek-chat`, via its OpenAI-compatible endpoint over stdlib urllib — no SDK); else `ANTHROPIC_API_KEY` → a Haiku-class fallback; else the deterministic fallback (keyword classification, template briefing). Classification requests strict JSON mode and tolerates trailing prose in the reply. The weekly briefing makes no LLM call. A monthly cost line appears in the first weekly briefing of each month.
